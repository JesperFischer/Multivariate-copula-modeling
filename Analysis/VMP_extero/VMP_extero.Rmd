...
pred_list <- future_lapply(subjects, function(s) {
  lapply(draws, function(d) {
    ...
    confidence_pred =  confidence_pred %>% mutate(ACC = ifelse(ACC == 1, "Correct","Incorrect")) %>% 
      pivot_wider(names_from = "ACC", values_from = pred) %>% unnest(Incorrect, Correct)
...
predictions %>% 
  ggplot() +
  geom_line(aes(x = x, y = prob, group = draw), alpha = 0.1)+
...
predictions %>% 
  ...
  summarize(RT_pred_mean = mean(RT_pred), 
            q20 = quantile(RT_pred,0.20), 
            q80 = quantile(RT_pred,0.80),
            q10 = quantile(RT_pred,0.10), 
            q90 = quantile(RT_pred,0.90),
            q60 = quantile(RT_pred,0.60), 
            q40 = quantile(RT_pred,0.40)
            ) %>% 
  ggplot() +
  geom_ribbon(aes(x = x, y = RT_pred_mean, ymin = q40, ymax = q60), alpha = 0.6)+
  geom_ribbon(aes(x = x, y = RT_pred_mean, ymin = q20, ymax = q80), alpha = 0.4)+
  geom_ribbon(aes(x = x, y = RT_pred_mean, ymin = q10, ymax = q90), alpha = 0.2)+
  geom_line(aes(x = x, y = RT_pred_mean), alpha = 0.5)+
  geom_point(data = df,
             aes(x = Alpha, y = DecisionRT), shape = 21, col = "black")+
  facet_wrap(~participant_id, scales = "free") +
  theme_classic(base_size = 16)


predictions %>% filter(draw %in% sample(1:1000,10)) %>% 
  ggplot() +
  geom_histogram(data = df %>% mutate(draw = NA),
             aes(x = DecisionRT, y = after_stat(density)),
             position = "identity",col = "black", alpha = 0.5)+
  geom_density(aes(x = RT_pred, group = draw), alpha = 0.01)+
  facet_wrap(~participant_id, scales = "free") +
  theme_classic(base_size = 16)
```
...
predictions %>% filter(draw %in% sample(1:1000,10)) %>% 
  ...
  ggplot() +
  geom_line(aes(x = x, y = Confidence, col = (ResponseCorrect), group = interaction(ResponseCorrect,draw)), alpha = 0.1)+
  geom_point(data = df %>% mutate(draw = NA) %>% mutate(ResponseCorrect = ifelse(ResponseCorrect == 1, "Correct","Incorrect")),
             aes(x = Alpha, y = Confidence / 100, fill = (ResponseCorrect), group = interaction(ResponseCorrect,draw)), shape = 21, col = "black")+
  facet_wrap(~participant_id, scales = "free") +
  theme_classic(base_size = 16)


predictions %>% pivot_longer(cols = c("Incorrect","Correct"), values_to = "Confidence",names_to = "ResponseCorrect") %>%
  group_by(x,participant_id,ResponseCorrect) %>% 
  summarize(Confidence_mean = mean(Confidence), 
            q20 = quantile(Confidence,0.20), 
            q80 = quantile(Confidence,0.80),
            q10 = quantile(Confidence,0.10), 
            q90 = quantile(Confidence,0.90),
            q60 = quantile(Confidence,0.60), 
            q40 = quantile(Confidence,0.40)
            ) %>% 
  ggplot() +
  geom_ribbon(aes(x = x, y = Confidence_mean, ymin = q40, ymax = q60, fill = (ResponseCorrect)), alpha = 0.6)+
  geom_ribbon(aes(x = x, y = Confidence_mean, ymin = q20, ymax = q80, fill = (ResponseCorrect)), alpha = 0.4)+
  geom_ribbon(aes(x = x, y = Confidence_mean, ymin = q10, ymax = q90, fill = (ResponseCorrect)), alpha = 0.2)+
  geom_line(aes(x = x, y = Confidence_mean, fill = (ResponseCorrect)), alpha = 0.5)+
  geom_point(data = df%>% mutate(ResponseCorrect = ifelse(ResponseCorrect == 1, "Correct","Incorrect")),
             aes(x = Alpha, y = Confidence / 100, fill = (ResponseCorrect)), shape = 21, col = "black")+
  facet_wrap(~participant_id, scales = "free") +
  theme_classic(base_size = 16)
