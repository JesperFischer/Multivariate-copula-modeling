...
```{r}
df0 = df %>% filter(t2duration == 0)

df0 %>% ggplot(aes(x = coh_single, y = cor))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df0 %>% ggplot(aes(x = coh_single, y = rt))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df0 %>% ggplot(aes(x = coh_single, y = conf,col = as.factor(cor)))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

mod0 = cmdstan_model(here::here("Stanmodels","Bin_RT_discrete_conf_3.stan"))


df0$sub = as.numeric(as.factor(df0$sub))

t_p_s = df0 %>% group_by(sub) %>% summarize(n = n())
...
datastan = list(N = nrow(df0),
              S = length(unique(df0$sub)),
              starts = starts,
              minRT = df0 %>% group_by(sub) %>% summarize(minRT = min(rt)) %>% .$minRT,
              ends = ends,
              t_p_s = t_p_s$n,
              X = df0$coh_single,
              ACC = df0$cor,
...
cor0$save_object(here::here("Saved models","Decender2020_discrete_conf_t0.rds"))



df1 = df %>% filter(t2duration == 1)

df1 %>% ggplot(aes(x = coh_single, y = cor))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df1 %>% ggplot(aes(x = coh_single, y = rt))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df1 %>% ggplot(aes(x = coh_single, y = conf,col = as.factor(cor)))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

mod = cmdstan_model(here::here("Stanmodels","Bin_RT_discrete_conf.stan"))


df1$sub = as.numeric(as.factor(df1$sub))

t_p_s = df1 %>% group_by(sub) %>% summarize(n = n())
...
datastan = list(N = nrow(df1),
              S = length(unique(df1$sub)),
              starts = starts,
              minRT = df1 %>% group_by(sub) %>% summarize(minRT = min(rt)) %>% .$minRT,
              ends = ends,
              t_p_s = t_p_s$n,
              X = df1$coh_single,
              ACC = df1$cor,
...
cor1$save_object(here::here("Saved models","Decender2020_discrete_conf_t1.rds"))
...
df = df %>% filter(t2duration == 2)

df %>% ggplot(aes(x = coh_single, y = cor))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df %>% ggplot(aes(x = coh_single, y = rt))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)

df %>% ggplot(aes(x = coh_single, y = conf,col = as.factor(cor)))+
  geom_jitter(height = 0.1,width = 0.05)+
  facet_wrap(~sub)
...
cor$save_object(here::here("Saved models","Decender2020_discrete_conf_t2.rds"))
