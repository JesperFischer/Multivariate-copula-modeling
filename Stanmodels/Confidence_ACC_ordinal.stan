...
data {
  ...
  array[N] int<lower=1, upper = Conf_level> Conf;
...
parameters {
  ...
  array[S] ordered[Conf_level-1] cuts;
...
model {
  ...
  for (n in 1:N) {
    ...
    u_mix[n, 3] = ordered_logistic_cdf(Conf[n] | conf_mu[n], cuts[S_id[n],]);

    target += lognormal_lpdf(RT[n] - rt_ndt[S_id[n]] | rt_int[S_id[n]] + rt_slope[S_id[n]] * entropy_t[n], rt_prec[S_id[n]]);

    target += ordered_logistic(Conf[n] |  conf_mu[n], cuts[S_id[n],]);
